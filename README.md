# ☸️ Preparação Certificação CKA - Luiz Antonio - Devopsproject - 2026

Repositório de estudos práticos e teóricos baseado no currículo oficial da Linux Foundation (2026).

## 🚀 Status do Aprendizado
- [x] 01. Course Introduction
- [x] 02. Basics of Kubernetes
- [x] 03. Installation and Configuration
- [x] 04. Kubernetes Architecture
- [ ] 05. APIs and Access
- [ ] 06. API Objects
- [ ] 07. Managing State with Deployments
- [ ] 08. Helm and Kustomize
- [ ] 09. Volumes and Data
- [ ] 10. Services
- [ ] 11. Ingress
- [ ] 12. Scheduling
- [ ] 13. Logging and Troubleshooting
- [ ] 14. Custom Resource Definitions
- [ ] 15. Security
- [ ] 16. High Availability
- [ ] 17. Exam Domain Review

## 🛠️ Detalhes do Lab (Notebook Zorin)
- **Ferramenta:** KVM/Libvirt
- **Cluster:** 1 Master + 2 Workers (Ubuntu 24.04)
- **Ferramentas extras:** kubectl, kubeadm, helm

## 📚 Referências Rápidas
- [Documentação Oficial K8s](https://kubernetes.io/docs/)
- [Cheat Sheet kubectl](https://kubernetes.io/docs/reference/kubectl/cheatsheet/)
https://devopsproject.com.br/
